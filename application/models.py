    ...
    if text:
        ...
        for chunk in chunks:
            try:
                ...
                return None
        # Join summaries and ensure each point is displayed on a new line
        return "\n".join([f"- {point.strip()}" for point in " ".join(summaries).split(".") if point.strip()])
    ...
    try:
        chat_completion = client_groq.chat.completions.create(
            messages=[
                {
                    "role": "user",
                    "content": f"Make the summary more precise into different points numbered in different lines. Number '1' in one line: {text}",
                }
            ],
            model=model,
        )
...
def calculate_bert_score(reference, hypothesis):
    try:
        P, R, F1 = calculate_bert_score(original_report_text, summary)
        logging.info(f"BERTScore - Precision: {P}, Recall: {R}, F1: {F1}")
    except Exception as e:
